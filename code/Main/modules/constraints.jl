module constrains
    export H2_constrains, electricity_constraints
    ## Everything is per kg of H₂ producted. So, 0 =  fully constraintd, 1 = unconstrained. 
    using CSV,DataFrames, XLSX, Statistics, Interpolations,JLD2
    ...
    """ 
    # Computes the hydrogen production constraints based on IEA data and IPCC AR6 scenarios.

    ## Description
    This function computes the constraints for hydrogen production technologies based on IEA data and IPCC AR6 scenarios. It calculates the share of each technology in the total hydrogen production and generates triangular distributions for each technology's cost share. The results are saved to a JLD file and plotted.

    ## methods
    """
    function H2_constrains()
        ...
        medians=[
            ...
            (vcat(median.(rand.(δc_NGccs,10000),dims=1)...).+ vcat(median.(rand.(δc_bioccs_h2,10000),dims=1)...)).*100,
        ...
        q05s=[quantile(cat(rand.(δc_PEM,10000)...,dims=2),0.05,dims=1) ⊙ 100,
            ...
            (quantile(cat(rand.(δc_NGccs,10000)...,dims=2),0.05,dims=1).+quantile(cat(rand.(δc_bioccs_h2,10000)...,dims=2),0.05,dims=1)) ⊙ 100,
            quantile(cat(rand.(δc_Coal,10000)...,dims=2),0.05,dims=1) ⊙ 100,
            quantile(cat(rand.(δc_Coalccs,10000)...,dims=2),0.05,dims=1) ⊙ 100,
            ]
        q65s= [ quantile(cat(rand.(δc_PEM,10000)...,dims=2),0.65,dims=1) ⊙ 100,
            quantile(cat(rand.(δc_AE,10000)...,dims=2),0.65,dims=1) ⊙ 100,
            quantile(cat(rand.(δc_SOEC,10000)...,dims=2),0.65,dims=1) ⊙ 100,
            quantile(cat(rand.(δc_biomass_h2,10000)...,dims=2),0.65,dims=1) ⊙ 100,
            # quantile(cat(rand.(δc_bioccs_h2,10000)...,dims=2),0.65,dims=1) ⊙ 100,
            quantile(cat(rand.(δc_NG,10000)...,dims=2),0.65,dims=1) ⊙ 100,
            (quantile(cat(rand.(δc_NGccs,10000)...,dims=2),0.65,dims=1).+quantile(cat(rand.(δc_bioccs_h2,10000)...,dims=2),0.65,dims=1)) ⊙ 100,
            quantile(cat(rand.(δc_Coal,10000)...,dims=2),0.65,dims=1) ⊙ 100,
            quantile(cat(rand.(δc_Coalccs,10000)...,dims=2),0.65,dims=1) ⊙ 100,
        ]
        ...
        q95s= [ quantile(cat(rand.(δc_PEM,10000)...,dims=2),0.95,dims=1) ⊙ 100,
            ...
            (quantile(cat(rand.(δc_NGccs,10000)...,dims=2),0.95,dims=1).+quantile(cat(rand.(δc_bioccs_h2,10000)...,dims=2),0.95,dims=1)) ⊙ 100,
        ...
        ## plotting
            ...
            title_letters = ["(a)", "(b)", "(c)", "(d)", "(e)", "(f)", "(g)", "(h)"]
                    for i ∈ 1:8
                        ...
                        axs[i].fill_between(years, q65s[i], q95s[i], alpha=0.4, label="65-95% range")
                        axs[i].fill_between(years, q05s[i], q95s[i], alpha=0.2, label="5-95% range")
                        axs[i].set_title(title_letters[i]*" "*titles[i], fontproperties=font_prop_labels)
                    ...
                    fig.tight_layout()
                    plt.savefig("../Source data/03_additional_data/4_00_constrains/hydrogen_constraints.svg", dpi=800, bbox_inches="tight",transparent=true)
                    plt.savefig("../Source data/03_additional_data/4_00_constrains/hydrogen_constraints.png", dpi=800, bbox_inches="tight")
                    display(fig)
                    plt.close("all")
        # end plot

        @save "../Source data/03_additional_data/4_00_constrains/hydrogen_constraints.jld" δc_PEM δc_AE δc_SOEC δc_biomass_h2 δc_bioccs_h2 δc_NG δc_NGccs δc_Coal δc_Coalccs

        return δc_PEM, δc_AE, δc_SOEC, δc_biomass_h2, δc_bioccs_h2, δc_NG, δc_NGccs, δc_Coal, δc_Coalccs
    end
    ...
    df_elec=filter(row -> row[:Model] !="IMAGE 3.0.1" && row[:Model] != "IMAGE 3.2" && row[:Model] != "REMIND", df_h2)
    
    SEᵉ= vcat(getVals("Secondary Energy|Electricity",years=years,df=df_h2),getVals("SE|Electricity",years=years,df=df_h2))
    """
    # Computes the electricity constraints based on IPCC AR6 scenarios.
    ## Description
    This function computes the constraints for electricity generation technologies based on IEA data and IPCC AR6 scenarios. It calculates the share of each technology in the total electricity mix and generates triangular distributions for each technology's cost share. The results are saved to a JLD file and plotted.
    ## methods
    """     
    function electricity_constraints()  

        ### electricity constrains % of future electrical mix
        SEᵉ= getVals("Secondary Energy|Electricity",years=years,df=df_elec)
        c_biomass_electricity = (getVals("Secondary Energy|Electricity|Biomass",years=years,df=df_elec) ⊘ SEᵉ) 
        ...
        c_gas_electricity = (getVals("Secondary Energy|Electricity|Gas",years=years,df=df_elec) ⊘ SEᵉ)
        ...
        c_gas_electricity_CC = (getVals("Secondary Energy|Electricity|Gas|w/ CCS",years=years,df=df_elec) ⊘ SEᵉ)
        ...
        c_gas_electricity_noCC = (getVals("Secondary Energy|Electricity|Gas|w/o CCS",years=years,df=df_elec) ⊘ SEᵉ)
        ...
        c_coal_electricity = (getVals("Secondary Energy|Electricity|Coal",years=years,df=df_elec) ⊘ SEᵉ)
        ...
        c_hydro_electricity = (getVals("Secondary Energy|Electricity|Hydro",years=years,df=df_elec) ⊘ SEᵉ)
        ...
        c_nuclear_electricity = (getVals("Secondary Energy|Electricity|Nuclear",years=years,df=df_elec) ⊘ SEᵉ)
        ...
        c_wind_electricity = (getVals("Secondary Energy|Electricity|Wind",years=years,df=df_elec) ⊘ SEᵉ)
        ...
        c_solar_electricity = (getVals("Secondary Energy|Electricity|Solar",years=years,df=df_elec) ⊘ SEᵉ)
        ...
        c_solar_PV_electricity = (getVals("Secondary Energy|Electricity|Solar|PV",years=years,df=df_elec) ⊘ SEᵉ)
        ...
        c_CSP_electricity = (getVals("Secondary Energy|Electricity|Solar|CSP",years=years,df=df_elec) ⊘ SEᵉ)
        ...
        c_geothermal_electricity = (getVals("Secondary Energy|Electricity|Geothermal",years=years,df=df_elec) ⊘ SEᵉ)
        max_c_geothermal_electricity=maximum(c_geothermal_electricity,dims=1)
        ...
        title_letters = ["(a)", "(b)", "(c)", "(d)", "(e)", "(f)"]
            ...
            fig.tight_layout()
            plt.savefig("../Source data/03_additional_data/4_00_constrains/electricity_constraints.svg", dpi=800, bbox_inches="tight",transparent=true)
            plt.savefig("../Source data/03_additional_data/4_00_constrains/electricity_constraints.png", dpi=800, bbox_inches="tight",transparent=true)
            display(fig)
            plt.close("all")
            # end plot

            return δc_biomass_electricity,δc_gas_electricity_CC, δc_gas_electricity_noCC, δc_coal_electricity, δc_hydro_electricity, δc_nuclear_electricity, δc_wind_electricity, δc_solar_electricity, δc_solar_PV_electricity, δc_CSP_electricity, δc_geothermal_electricity
    end
   electricity_constraints()   
end